...
```{r}
state_coverage1314 <- coverage1.6 %>%
  filter(Location != "District of Columbia", Location != "United States")
state_coverage1314
state_spending1314 <- spending1.6 %>%
  filter(Location != "District of Columbia", Location != "United States")
state_spending1314
```
...
```{r}
#coverage2.1 <- coverage1.7 %>%
 # select(ends_with("Employer"))
#coverage2.1
g <- ggplot(data = healthcare, aes(x = log(spending), y = log(coverage)))
```
...
healthcare.3.1
  ggplot(data = healthcare.3.1, 
         mapping = aes(x = total_population/10000000, 
                       y = total_health_spending/1000)) + 
          geom_point() + 
          labs(x = "Total Population in Millions", 
               y = "Total Spending in Thousands")
```
There is a very strong positive relationship between total population and total spending.
...
The US spends nearly $10,000 on healthcare for every member of its population, yet less than half of Americans receive healthcare coverage from their employers. However, this relationship varies by region. This figure reveals that spending per capita is highest in the Northeast and lowest in the West, while employer coverage per capita is highest in the Northeast and North Central regions. Thus, North Central US yields the lowest spending to coverage ratio and may serve as a valuable case study for other US regions.

```{r}
healthcare.3.2 <- healthcare.3.1 %>%
  mutate(spending_per_capita = total_health_spending*1000000/total_population) %>%
  inner_join(state_info, by = c("location" = "location"))
glimpse(healthcare.3.2)

  ggplot(data = healthcare.3.2, 
         mapping = aes(x = employer/total_population, 
                       y = (spending_per_capita))) + 
          geom_point(aes(color = region)) +
          facet_wrap(~year) +
          labs(x = "Proportion of Population with Employer Coverage", 
               y = "Spending per Capita (Dollars)")
```
...
Alaska spent the most per capita on health care in 2013 and 2014. Utah spent the least in both years.

```{r}
healthcare.4.1 <- healthcare.3.2 %>%
  arrange(desc(spending_per_capita)) %>%
  select(location, year, spending_per_capita)
healthcare.4.1
```
...
```{r}
ggplot(data = healthcare.3.2) +
   geom_freqpoly(aes(x = spending_per_capita)) +
   facet_wrap(~region)

mosaic::favstats(~spending_per_capita|region, data = healthcare.3.2)
```
