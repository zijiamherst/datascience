...
Coverage covers the years 2013-2016, spending covers the years 1991-2014. The "Other Public" column are stored as chr, possible because of the NAs in that column.
...
```{r}
coverageSmall <- coverage %>%
  select(starts_with(c("Location","2013","2014")))
spendingSmall <- spending %>% 
  select(starts_with(c("Location","2013","2014")))
```
...
I would expect a positive correlation between spending and coverage. The x axis would be coverage and spending on the y axis. Each points would represent a year in a state. The dataset needs to be in one data frame with the locations of both line up and corresponding columns of spending and coverage per year. The rows would have the states. It will also require a new column for the year
 
## <!-- 2.2 -->What are some of the steps that will need to be taken to get the data in that form?  

I will need to inner_join() the two datasets by creating a new dataset.
...
```{r}
coverageSmall
healthcare <- coverageSmall %>%
  inner_join(spendingSmall, by = "Location") %>% 
...
```{r}
healthcare %>%
  ggplot(mapping = aes(x = total_population, y = employer/1000)) +
  geom_point(aes(color = year)) + 
  scale_x_continuous(n.breaks = 10) +
  labs(title = "Employer Coverage versus Population Size",
       x = "Total population in millions",
       y = "Employer coverage in thousand dollars")
healthcare %>%
  ggplot(mapping = aes(x = total_population/1000000, y = total_health_spending/1000)) +
  geom_point(aes(color = year)) + 
  scale_x_continuous(n.breaks = 10) +
  labs(title = "Total Spending versus Population Size",
       x = "Total population in millions",
       y = "Total spending in thousand dollars")
```

While both exhibit a positive correlation, employer coverage is much steeper than total health spending and the y axis labelings show that. Employer coverage is from 0 - 15,000k while health spending ranges from 0 - 300k.
...
ggplot(data = healthcare) + 
  geom_point(aes(color = region,
                 x = employer,
                 y = (total_health_spending*1000000/total_population))) + 
  facet_wrap(~year) + 
  labs(title = "Spending Per Capita versus Proportion With Employer Coverage",
       x = "Employer Coverage",
       y = "Total Health Spending Per Capita")

```

In a lot of states it seems like the employer coverage does not accurately reflect the needs of its citizens. With the left leaning points the data shows that employer coverage is often very low, irregardless the needs of the constituents of their states, high or low. The lack of a clear correlation indicates that employer coverage isn't calculated based on per capital spending.
