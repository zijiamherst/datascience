...
```{r}
healthcarespending <- coverageSmall %>%
  select(ends_with(c("Location","employer"))) %>%
  inner_join(spendingSmall, by = c("Location" = "Location")) %>%
  clean_names() %>%
  pivot_longer(-location ,names_to = "year", values_to = c("spending","insurer") )
healthcarespending
```
